...
- Canonical active-branch IDs/hashes and supported Pi events establish source authority. Preappend `message_end`, raw tools, siblings and thinking do not.
...
- Strict v5 checkpoints persist bounded generated labels, refs, assessment scalars, events, journals, usage and timestamps. No v4 migration, raw conversation, prompts, provider envelopes or credentials. Canonical amendments invalidate stale derived references. Local checkpoints are trusted writable state, not an adversarial security boundary.
