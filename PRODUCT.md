...
- Main-session only. Never execute tests/commands, mutate Beads, block tools, cancel reviews or follow child workers. The implemented reconciliation capability may inject a visible question and trigger a selected-model response under master ON; this can incur additional model/Jev/extraction charges.
...
Owner-authorized advisory epic `pi-progress-barroot-q0n` implements reconciliation locally; full local acceptance and human manual acceptance are distinct. Test-writing and review-correction capabilities remain pending their adapter/evidence gates, not shipped. Existing health now judges long-term regression value using the same necessity question; explicit test requirements and existing validation remain binding. Claude/Codex/OpenCode integrations and execution-visibility redesign are outside this advisory candidate. The [canonical advisory contract](docs/design/advisory-nudges.md) governs remaining delivery work; no release/install/push is authorized.
