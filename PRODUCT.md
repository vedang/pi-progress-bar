...
- Expose only `/progress`, `/progress on`, and `/progress off`; bare command shows usage/state, not a menu. Remove manual source/scope/current-task selection and details/enable/pause/resume.
- Canonical Pi branch observations and relevant tool evidence coalesce bounded analysis work. Idle redraws, clocks, unchanged snapshots, and monitoring entries never dispatch inference; finite valid work drains with one Jev request in flight and a yielded local turn between dispatches. Newly observed user candidates retain exact references in a bounded fresh lane; while historical work is eligible, neither lane receives more than two consecutive dispatches.
...
- Existing widget and bare `/progress` output must distinguish catch-up, unresolved scope, unknown current work, no new evidence, and transport/controller rejection using bounded aggregate reason codes only. Bare help also surfaces current service error and retry/backoff state. They show `Last Jev call` from actual HTTP dispatch (including failed calls), never a cache/redraw or accepted-result proxy; `Never` is truthful before dispatch. No raw conversation, credentials, prompts, model answers, retained task-card text, or dispatch timestamp appear in diagnostics/checkpoints. Retained display cards are memory-only, task/revision bound and discarded if their source is unavailable on the active branch. Reload journals retain canonical IDs/ranges and derived enums only; strict schema v4 requires every saved source/task work kind and rejects v3 or malformed shapes without migration. Their unkeyed digest detects accidental or unrecomputed corruption, not deliberate writable-checkpoint edits that recompute it. Local saved session state is trusted on the same boundary as restored ledger statuses/proofs.
