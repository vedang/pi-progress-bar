...
it("Pi 0.84.2 exposes canonical users at context during an active steering turn, and assistants at turn_end", async () => {
  ...
  const loader = new DefaultResourceLoader({
    ...
    extensionFactories: [
      (pi) => {
        ...
        pi.on("agent_settled", (_event, ctx) => observe("agent_settled", ctx));
      },
    ],
  });
  await loader.reload();
  ...
  try {
    ...
    expect(
      snapshots.find((snapshot) => snapshot.hook === "message_end:user")?.refs,
    ).toEqual([]);
    ...
    for (const snapshot of snapshots) {
      ...
      for (const ref of snapshot.refs) expect(final?.refs).toContainEqual(ref);
    }
    const file = manager.getSessionFile();
    ...
    expect(
      disk.filter((entry) => entry.type === "message").map((entry) => entry.id),
