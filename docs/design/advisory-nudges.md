...
| `src/sources/messages.ts` admits user, assistant, and exact `intercom_message` observations, excludes other custom entries, and excludes aborted/error assistant messages. | A custom advisory itself is not canonical task input, but an assistant echo can be. H1/S0 decide smallest evidence-consumer proof before delivery. |
...
3. The question itself, copied task labels, or a copied directive are not completion/new-task evidence. H1 must prove exact full echo, copied directive/rows, independent status, mixed answer, pending replay, and no-rebilling behavior. S0 closes S1/S2 as not needed if current contracts suffice; otherwise it returns one smallest named request/proof/replay/checkpoint change for owner approval before S1/S2.
...
- **H0** proves source event/order/host surface. **H1** proves response evidence contract. **R0** freezes finite live retry/reload behavior from H0 measurements. **S0-S2** are conditional evidence work, never automatic expansion.
...
- A6 implements only tested delivery/lifecycle wiring and any H1/S0-approved evidence seam. It exposes no advisory control and preserves v8.
...
No claim in this document proves H0 host behavior, H1 echo separation, R0 retry constants, B0 action binding, C0 review adapter, provider admission need, or a capability acceptance. Those are explicit future investigations/gates. The only fixed delivery time in this revision is the owner-approved 60,000-ms reconciliation deadline; R0 chooses finite retry values from host evidence.

## 9. Canonical citations
