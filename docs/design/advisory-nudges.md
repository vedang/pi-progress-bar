...
H0 is characterized on both hosts, with limits recorded in section9; this is not capability acceptance. H1 found a copy-only evidence gap and returned a prerequisite to S0. Section9 freezes R0 engineering retry constants separately from the owner-approved60,000-ms reconciliation deadline. Owner resolved S0 by declining additional evidence machinery and retaining existing Jev processing; S1/S2 are not needed. A7 is locally accepted. B0 is supported for attempted builtin starts by explicit owner decision. T1/T2 are not needed: existing bounded gateway machinery is reused, without a provider-wide scheduler redesign. B/C are locally accepted at `203d1da8`, independently cleared by review `f419d006`. Gates: 840 unit tests; pinned host 61 integration passes plus 8 expected skips; installed host 69 passes. Production-shaped semantic calibration and actual async-producer/parent-timing proofs are retained in local task artifacts. Owner manual acceptance remains pending; no release/install/push is authorized.
...
- B returns an attempted action immediately. C stores only the exact named asynchronous `review` declaration from the installed git pi-subagents package, then requires its own parent tool-call ID, workflow run ID, resolved named-resource provenance and matching real workflow/async IDs from successful `tool_execution_end`. An empty child inventory is valid at logical workflow start; no child run ID is fabricated. Declarations alone never trigger C. Observed `subagent:async-complete` revokes the admitted workflow; initial sends and retries require its active identity and originating main run. Foreground calls, raw scripts, generic agents and mismatched receipts abstain. The adapter does not start or cancel anything.
...
- Monitor owns role-tagged bounded conversation context, full copied board, current accepted health facts, usage accounting and post-await freshness. Corrective requests additionally carry complete bounded loaded policy (8 KiB) and finalized assistant action/batch (4 KiB); missing or overflowing authority abstains. Direct user requests override cached necessity and cannot be waived by assistant/intercom suggestions. Snapshot identity is a fixed digest; replacement health revokes old eligibility synchronously. Delivery remains bound to the active originating run, latest admitted correction-attempt identity, and exact snapshot on every attempt. New distinct B/C attempts revoke older classifier emissions/retries; duplicate starts and blocked B tool ends do not. A separate optional `JevGateway` reuses existing bounded provider behavior; there is no global one-flight guarantee. Inputs, navigation, shutdown, OFF and changed identity revoke stale inference. No checkpoint fields change.
