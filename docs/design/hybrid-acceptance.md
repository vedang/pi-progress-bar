...
Accepted runtime: **`e4dcfe08`**, jj change **`ztmyrzkzvqonqtlyopknyywxotrmwvoq`**. Main independently passed format, TypeScript, Biome, Knip, **378 unit +12 integration tests**, **12 global-host integration tests**, and package dry-run. Local host is Pi0.84.2/pi-ai0.84.4; global host is Pi0.85.1. Independent cumulative review **`afdbee79..e4dcfe08` PASS**, with no outstanding findings. This accepts the repair for publication and manual QA; it does not claim manual QA has passed.

### Latest repair evidence

Final sources `c90ca095` and `e4dcfe08` close pending-context and barrier lifecycle races: exact OFF→ON context checks, resolve-only cancellation, owner/epoch fences, old-branch owner detachment before target validation, and same-owner barrier replacement checks across synchronous and microtask orderings. Main froze seven additional failing regressions before these repairs. All20 focused lifecycle cases pass. Final evidence: `eighth-main-*.log` and `eighth-repair-review.md` in the task directory. Earlier blocked reviews below are historical and superseded by this cumulative PASS.

Review of `7a3fca75` found continuation/control regressions despite green gates. Main `04d0017c` reproduced11 failures with two continuous/reload controls. Source `ba24602c` unifies restore/enable control intent, awaits target restoration, preserves OFF intent, freezes active request context behind an authority barrier, removes competing backward-latest scanning, preserves same-session tree telemetry, and clears terminal catch-up. All regressions and full gates above pass; logs `sixth-main-*.log`. The final cumulative review accepts this repair. No new paid calls were used for these local invariant repairs.

The next cumulative review judged `124ee557`'s paid admission/replay repairs sound, but found three further blockers: OFF/local-save capacity, skipped blank/thinking candidate traversal, and telemetry reset on same-source amendment. Source `21a2a6d2` plus `7a3fca75` repairs these and two main-discovered frontier invalidation bugs. Main gates above passed; logs are `fifth-final-*.log`. The final cumulative review accepts these repairs. An existing exact-limit ON-only checkpoint now fails safely OFF without losing its valid accepted journal; newly committed states must fit OFF. Same-source semantic invalidation preserves incurred usage and dispatch timestamps.
...
Artifacts in `.agents/plans/20260919T191131--enable-useful-implementation-health__active/`: `health-live-1789826237763.jsonl` (failed), `health-live-1789826417885.jsonl` (passed), `main-gates.log`, `applicability-main-gates.log`. These finite fixtures demonstrate the new choices, not universal semantic accuracy. The final cumulative acceptance above supersedes the earlier outstanding full-batch blockers.
