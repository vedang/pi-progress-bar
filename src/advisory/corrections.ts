      ...
      board.map((task, index) => [
        `correct:${task.id}`,
        {
          ...
          criteria: {
            ...
            required:
              attempt.kind === "test"
                ? "An explicit user request or applicable repository/system policy requires this NEW failing test. This takes priority over cached not-needed health; do not correct it."
                : "An explicit user request, repository/system rule, or genuine security/audit/blocker need requires this review NOW; incomplete implementation does not override that requirement.",
            unrelated:
              attempt.kind === "test"
